...
class Tpl{
    ...
    protected $lifeTime = 3600; //设置缓存过期时间
    protected $vars = []; //用来存放显示变量数组
    //构造方法对成员变量进行初始化
    public function __construct($viewDir = null, $cacheDir = null, $lifeTime = null)
    {
        //判断是否为空，如果为空，则使用默认值，如果不为空，判断并且设置
        if (!empty($viewDir)){
            //判断锁传递的路径是否为目录
    ...
    //分配变量方法，例如：$title = '中国'; $tpp -> assing('title', $title);
    ...
    /**
     * @param $viewName 模板文件名
     * @param bool $isInclude 模板文件是仅仅需要编译，还是先编译再包含进来
     * @param null $uri 例如：index.php?page=1 为了让缓存的文件名不重复，将文件名和uri拼接起来再md5一下，生成缓存的文件名
     */
